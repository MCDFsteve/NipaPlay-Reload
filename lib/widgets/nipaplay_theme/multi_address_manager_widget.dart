...
class _MultiAddressManagerWidgetState extends State<MultiAddressManagerWidget> {
  ...
  Future<void> _showAddAddressDialog() async {
    ...
    final result = await showDialog<Map<String, String>>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        title:
            Text('添加服务器地址', style: TextStyle(color: _foregroundColor(context))),
        content: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                labelStyle: TextStyle(color: _foregroundColor(context, 0.7)),
                hintStyle: TextStyle(color: _foregroundColor(context, 0.3)),
          ...
          TextButton(
            onPressed: () {
              if (_urlController.text.trim().isNotEmpty) {
                ...
                final name = UrlNameGenerator.generateAddressName(url,
  ...
  Future<void> _confirmRemoveAddress(ServerAddress address) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        backgroundColor: Colors.black.withOpacity(0.8),
        title: Text('删除地址', style: TextStyle(color: _foregroundColor(context))),
        content: Text(
  ...
  Future<void> _showPriorityDialog(ServerAddress address) async {
    ...
    final result = await showDialog<int>(
      ...
      builder: (dialogContext) => AlertDialog(
        backgroundColor: Colors.black.withOpacity(0.8),
        title:
            Text('设置优先级', style: TextStyle(color: _foregroundColor(context))),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '• 0: 最高优先级（优先）\n• 1-3: 高优先级\n• 4-9: 中等优先级\n• 10+: 低优先级',
                    style: TextStyle(
                        color: _foregroundColor(context, 0.7), fontSize: 12),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildAddressStatus(ServerAddress address) {
    ...
    if (address.lastSuccessTime != null) {
      ...
      return Text(
        '上次成功: $timeText',
        style: TextStyle(color: _foregroundColor(context, 0.5), fontSize: 12),
      );
    }
  ...
  Widget _buildPriorityBadge(ServerAddress address) {
    ...
    if (address.priority > 0 && widget.addresses.length > 1) {
      return Container(
        ...
        child: Text(
          'P${address.priority}',
          style: TextStyle(color: _foregroundColor(context, 0.6), fontSize: 10),
        ),
      );
    }
          ...
          child: Builder(
            builder: (context) {
              ...
              final sortedAddresses =
                  List<ServerAddress>.from(widget.addresses);
              sortedAddresses.sort((a, b) {
                // 当前使用的地址优先显示
                if (a.id == widget.currentAddressId &&
                    b.id != widget.currentAddressId) return -1;
                if (b.id == widget.currentAddressId &&
                    a.id != widget.currentAddressId) return 1;
          ...
          child: Row(
            children: [
              Icon(Icons.info_outline,
                  color: Colors.blue.withOpacity(0.7), size: 16),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
